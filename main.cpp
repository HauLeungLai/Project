...
void buyStock(const std::string& currentUser) {
    ...
    if (choice == 'l') {
        printw("\nEnter limit price: ");
        echo();
        if (scanw("%lf", &priceToBuy) != 1) {
            printw("Invalid input for price.\n");
            getch();
            return;
        }
        noecho();
    ...
    echo();
    int scanResult = scanw("%d", &quantity);
    noecho();
    printw("Scan result: %d, Entered quantity: %d\n", scanResult, quantity);
    getch(); // Wait for user input before proceeding

    if (scanResult != 1 || quantity <= 0) {
        printw("Invalid input for quantity.\n");
        getch();
        return;
    }

    double totalCost = priceToBuy * quantity;
    printw("Total cost: $%.2f\n", totalCost);
    printw("Available cash: $%.2f\n", portfolio.getAvailableCash());
    getch(); // Wait for user input

    if (portfolio.getAvailableCash() >= totalCost) {
        try {
            printw("Creating Stock object...\n");
            getch();
            auto newStock = std::make_unique<Stock>(ticker, ticker, priceToBuy);
            printw("Stock object created.\n");
            getch();

            printw("Adding asset to portfolio...\n");
            getch();
            portfolio.addAsset(std::make_unique<Stock>(ticker, ticker, priceToBuy), quantity);
            printw("Asset added to portfolio.\n");
            getch();

            printw("Updating available cash...\n");
            getch();
            portfolio.setAvailableCash(portfolio.getAvailableCash() - totalCost);
            printw("Available cash updated.\n");
            getch();

            printw("Successfully bought %d shares of %s.\n", quantity, ticker.c_str());
        } catch (const std::exception& e) {
            printw("Error during portfolio operations: %s\n", e.what());
            getch();
        } catch (...) {
            printw("Unknown error during portfolio operations\n");
            getch();
        }
    } else {
        printw("Insufficient funds.\n");
    }

    printw("Saving portfolio...\n");
    getch();
    try {
        portfolio.savePortfolio(currentUser + "_portfolio.dat");
        printw("Portfolio saved successfully.\n");
    } catch (const std::exception& e) {
        printw("Error saving portfolio: %s\n", e.what());
    } catch (...) {
        printw("Unknown error saving portfolio\n");
    }
...
void sellStock(const std::string& currentUser) {
    ...
    if (!portfolio.hasAsset(ticker, quantity)) {
    ...
    try {
        ...
        portfolio.sellAsset(ticker, quantity);
        ...
        switch (choice) {
            case 1:
                if (loginUser(users, currentUser)) {
                    portfolio.loadPortfolio(currentUser + "_portfolio.dat");
